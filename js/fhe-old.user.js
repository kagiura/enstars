...
function tagDialogue(){
        ...
        if( newUnit || rich ){
            ...
            if( !$(cache).hasClass('qd-exclude') ){
                ...
                if( $(cache).is("[character]") ){
                    ...
                    $(unit).append( $('<img></img>').attr(`src`, iconFile) );
                    $(unit).append( $('<span></span>').text(thisName) );
                }
                $(unit).append(cache);
            ...
            if( newUnit ) {
                cache = $(`<div></div>`).addClass(`qd-lines`);
                $(cache).attr('character', name)
                $(cache).append(`<p>` + $(this).html() + `</p>`);
            } else {
                cache = $(`<div></div>`).addClass(`qd-rich`);
    ...
    inlineNotes.each(function() {
        $(this)
        .prepend('<span class="material-icons-round">description</span>')
